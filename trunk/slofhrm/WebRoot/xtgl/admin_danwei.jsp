...
<html>
	...
	<body topmargin="1" leftmargin="0">
        <script language=javascript>
            ...
            document.form1.action="delete_danwei_tj.jsp";
        ...
        <form name="form1" method="post">
            ...
            <TR>
                <TD class="heading" bgColor="#FFDAB9" height="20"><DIV style="font-size:15px;filter:progid:DXImageTransform.Microsoft.Glow(Color=#FFFFFF,Strength=3);width:200px"><font color="#4B0082">&nbsp;&nbsp;单位维护</DIV></TD>
            </TR>
            <TR>
            <TD vAlign="top" height="120">
                ...
                <TR bgcolor="#FFEFD5" align="center" height="25px">
                    ...
                    <TD width="50%" bgcolor="#FFEFD5"><STRONG>单位名称</STRONG></TD>
                    <TD width="45%" bgcolor="#FFEFD5"><STRONG>单位描述</STRONG></TD>
...
<%
		...
		if (roleid.equals(admin_role_id)){
			...
			//判断是否有下级单位。
			int count_num=0;
			ResultSet count_rs2 = qxbean.getResultSet("select count(*) as num from GetSubdanwei(1,"+zhu_id+")");
			if(count_rs2.next()){count_num=Integer.parseInt(count_rs2.getString("num"));}
			...
			<TR id="tr_<%= zhu_id %>_<%= superior_id %>_<%= lvl %>" bgcolor="#FFFAFA" <%if (Integer.parseInt(lvl)==sub_admin_lvl){%>style="display:inline"<%}else{%>style="display:none"<%}%>>
				<TD align="center"><%if(count_num>1){%><input type="checkbox" name="chkbx" id="chkbx_<%= zhu_id %>_<%= superior_id %>_<%= lvl %>" value="<%=id%>" onclick="selectitem(this,<%= zhu_id %>,<%= id %>,<%= lvl %>)"><a id="a_<%= zhu_id %>_<%= id %>_<%= lvl %>"></a><%}%></TD>
				<TD><a id="a_<%= zhu_id %>_<%= id %>_<%= lvl %>" onclick="showitem(<%= zhu_id %>,<%= id %>,<%= lvl %>)" style="hover{COLOR: red; TEXT-DECORATION: none}; cursor:hand"><%for(i=0;i<intlvl;i++){out.println("<img src='img/space_line.gif' align='absmiddle'>");}%><%if (HasSub==0){out.println("<img src='img/open_line.gif' align='absmiddle'><img src='img/zi.gif' align='absmiddle'>");}else{out.println("<img id='img_"+zhu_id +"_"+id +"_"+lvl+"' src='img/close.gif' align='absmiddle'><img src='img/zhu.gif' align='absmiddle'>");}%></a><A Href = "edit_danwei.jsp?id=<%=id%>"><%=name%></a></TD>
				<TD><%= description %></TD>
			</TR>
			<%
                ...
                <table style="BORDER-COLLAPSE: collapse" borderColor="#111111" cellSpacing="1" cellPadding="0" width="100%" border="0">
                    <TR>
                        ...
                        <TD width="15%" align="center" bgColor="#FFEFD5" height="24">
                            <input type="button" name="adddanwei" value="添加" onclick="window.location='add_danwei.jsp'" />&nbsp;&nbsp;
