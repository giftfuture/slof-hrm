...
<html>
	...
	<body topmargin="1" leftmargin="0">
        ...
        <form action="add_role_tj.jsp" method="post" name="form1">
            <table height="138" cellSpacing="0" cellPadding="0" width="100%" border="0">
            <TR>
                <TD class="heading" bgColor="#FFDAB9" height="20"><DIV style="font-size:15px;filter:progid:DXImageTransform.Microsoft.Glow(Color=#FFFFFF,Strength=3);width:200px"><font color="#4B0082">&nbsp;&nbsp;用户组信息录入</DIV></TD>
            </TR>
            <TR>
                <TD vAlign="top" >
                    ...
                    <TR>
                        ...
                        <TD bgColor="#ffffff" ><input name="rolename" type="text" size="50" maxlength="50" id="rolename"/><FONT face="宋体" color="#ff0000">*</FONT></TD>
                    </TR>
                    <TR>
...
if (roleid.equals(admin_role_id)){
        ...
        <select name="superiorid">
            ...
            while(zhu_rs.next()){
                ...
                while(rs.next()){
					...
					%>                                
                    <option value="<%=id%>"><%for(i=0;i<intlvl;i++){out.println("|");}%><%if (HasSub==0){out.println("-");}else{out.println("+");}%><%=name%></option>
                <%}
             }%>
        </select>
        <FONT face="宋体" color="#ff0000">*</FONT>
        <input type=hidden name="return_flag" value="ToMain">
    <%}else{
            danwei_id = request.getParameter("id");
            ResultSet rs = qxbean.getResultSet("select * from xtglt_danwei where id='"+danwei_id+"'");
            if(rs.next()){
                name=rs.getString("name");
            }%>
            <input type=text name="danweiid" readonly value="<%=name%>"><input type=hidden name="superiorid" value="<%= danwei_id %>"><input type=hidden name="return_flag" value="ToSub">
    <%}
}else{
    ...
    <%}
                    ...
                    <TR>
                        ...
                        <TD bgColor="#ffffff"><textarea name="note" rows="6" id="note" style="height:72px;width:456px;"></textarea></TD>
                    </TR>
                    <TR>
        ...
        HasSub=0;
            ...
            while(zhu_rs.next()){
                ...
                //功能中不包含功能菜单管理权限，xtglt_gn表中默认的功能菜单ID=2
                ResultSet rs2 = qxbean1.getResultSet("select * from  GetSubgn_ByRoleID(1,"+zhu_id+","+roleid+") where id<>'2'");
                while(rs2.next()){
                    id=rs2.getString("id");
