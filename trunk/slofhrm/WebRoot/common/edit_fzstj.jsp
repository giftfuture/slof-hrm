...
<html>
    ...
    <script language=javascript>
        ...
        {alert("你必须选择报告类别!");document.form1.bglb.focus();return;}
            ...
            document.form1.action="edit_fzstj_tj.jsp";
            document.form1.submit();}
        }

		function query(){
			document.form1.action="edit_fzstj.jsp";
			document.form1.submit();
		}
...
<%
    ...
    String tempurl="common/admin_fzstj.jsp";
%>
...
if (request.getParameter("id")!=null){
	...
	ResultSet jcxmtj_rs = qxbean.getResultSet("select * from commont_fzstj where id='"+ jcxmtj_id +"'");
	if(jcxmtj_rs.next()){
...
ResultSet count_rs = qxbean.getResultSet("select count(*) num from commont_fzstj_fzs where jcxmtj_id='"+jcxmtj_id+"'");
if(count_rs.next()){
	count=Integer.parseInt(count_rs.getString("num"));
}
String[] aryjcxm_id=new String[count];
ResultSet jcxmtj_fzs_rs = qxbean.getResultSet("select jcxm_id from commont_fzstj_fzs where jcxmtj_id='"+jcxmtj_id+"'");
while(jcxmtj_fzs_rs.next()){
	aryjcxm_id[j]=jcxmtj_fzs_rs.getString("jcxm_id");
	j++;
}
%>
			...
			<TABLE style="BORDER-COLLAPSE: collapse" cellSpacing="1" width="100%" bgColor="#666666" border="0">
				...
				<TR>
					...
					<TD bgColor="#ffffff">
						<select name="bglb" id="bglb" onchange="query()">
							...
							ResultSet bglb_rs = qxbean.getResultSet("select distinct bgzl2_id, name from all_bg inner join commont_bgzl2 on commont_bgzl2.id=bgzl2_id where commont_bgzl2.name like '%水%' and danwei_id in (select id from getsubdanwei(1,"+ danwei_id +"))");
							while(bglb_rs.next()){
					...
					<TD bgColor="#ffffff">
						...
						if(!bglb.equals("")){
							...
							<TR>
								...
								<TD align="left">&nbsp;<input type=checkbox name="chkbx" id="chkbx" <%if (Has_fzs.equals("YES")){%>checked<%}%> value="<%=jcxm_id%>"><%=jcxm_name%></TD>
							</TR>
							<%
							}
						}
				...
				<TR>
					...
					<TD bgColor="#ffffff" height="18">
						...
						<input type="button" name="return" value="返回" onclick="window.location='admin_fzstj.jsp'" />
					</TD>
				</TR>
			</TABLE>
		</TD>
