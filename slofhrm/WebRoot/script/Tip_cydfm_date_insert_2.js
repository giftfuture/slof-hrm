...
function ShowTip(inpt,tmpstring,x1,y1) 
{
...
function disp(x1,y1) {
	if ( (ns4) || (ie4) ) {
		if (show == 0||show == 1) 	{
			if (dir == 2) { // Center
				moveTo(over,x+x1-(width/2),y+y1);
			}
			if (dir == 1) { // Right
				moveTo(over,x+x1,y+y1);
			}
			if (dir == 0) { // Left
				moveTo(over,x-x1-width,y+y1);
			}
			showObject(over);
			show = 1;
		}
